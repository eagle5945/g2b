...
fileHandler = logging.handlers.RotatingFileHandler(filename='/home/ubuntu/g2b_bot/log/noti_bot.log', maxBytes=file_max_bytes, backupCount=10)
...
def pushHrcspSsstndrdInfoService(user_id, list_command):
    ...
    sql = f"SELECT A.* FROM HrcspSsstndrdInfoService A LEFT JOIN (SELECT * from NotiHistoryHrcsp WHERE USER = '{user_id}') B "\
        ...
        f"WHERE B.bfSpecRgstNo IS NULL AND {swBizObjYn} "\
...
def push_search_info(df):
    for idx in range(len(df)):
        ...
        mylog.info(f"{user_id} : {df['command'][idx]}")
